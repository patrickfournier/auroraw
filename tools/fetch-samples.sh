#!/usr/bin/env bash
# Downloads the CC0 RAW samples (raw.pixls.us) used by the tests and benchmarks into testdata/samples.
...
fetch() { # <path on the server> <local name>
  [ -s "testdata/samples/$2" ] && { echo "have $2"; return; }
  echo "fetching $2"
  curl -fsSL --retry 3 -o "testdata/samples/$2" "$base/$1"   # -L: the server redirects to /download/
}
