...
| D-082 | 2026-09-20 | **Governance adopted** (docs/governance.md). **Contributions**: the Developer Certificate of Origin (`Signed-off-by` on every commit, checked on pull requests); commits made by Claude Code carry the owner's sign-off. **Licence**: GPL-3.0-or-later. **Plugins** (resolves D-057): a plugin that uses only the plugin API may have any licence, stated in an additional permission under section 7 of the GPL (`PLUGIN-EXCEPTION`, to be reviewed by a qualified person before the first release); the tie-break of D-080 applies. **Language**: everything, including discussions, is in English. **Translations**: `.po` files sent as pull requests. **Conduct**: Contributor Covenant 2.1, reports to auroraw-conduct@straycat.ca. **Security**: private vulnerability reporting on. **Funding**: GitHub Sponsors button. |
